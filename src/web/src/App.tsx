...
type View = "mailboxes" | "inbox" | "settings";
...
function readInitialView(): View {
  ...
  return saved === "inbox" || saved === "mailboxes" || saved === "settings" ? saved : "mailboxes";
}
...
export function App() {
  ...
  const [accountHelp, setAccountHelp] = useState<Record<string, string>>({});
  ...
  async function handleCreateMailbox() {
    setStatus(""); setError("");
    try {
      const created = await createMailbox(suffix, selectedAccountId);
      setSuffix("");
  ...
  async function handleDeleteAccount(account: Account) {
    ...
    if (!confirm(`删除主账号 ${root || account.name || account.id}？本地会停止显示该账号。`)) return;
    setStatus(""); setError("");
    try {
  ...
  async function handleMoveAccount(account: Account, direction: -1 | 1) {
    const ordered = [...accounts].sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0) || a.id - b.id);
    const index = ordered.findIndex((item) => item.id === account.id);
    const target = index + direction;
    if (index < 0 || target < 0 || target >= ordered.length) return;
    [ordered[index], ordered[target]] = [ordered[target], ordered[index]];
    setAccounts(ordered.map((item, order) => ({ ...item, sort_order: order * 10 })));
    try {
      await Promise.all(ordered.map((item, order) => updateAccount(item.id, { sort_order: order * 10 })));
    } catch (err) {
      reportError(err);
      loadAccounts().catch(reportError);
    }
    ...
    return (
      <main className="login-shell">
        ...
        <section className="stage">
          ...
          <div className="stamp">
            {t("login.stamp.session")} 路 {stamp} utc+8
            <span style={{ marginLeft: 14, color: "var(--accent-fg)" }}>鈼?{t("login.stamp.online")}</span>
          </div>
        </section>
  ...
  return (
    <main
      className="app-shell"
      style={{ "--rail-width": rail.width + "px" } as any}
    >
      <aside className="rail">
        ...
        <nav>
          ...
          <button className={view === "mailboxes" ? "active" : ""} onClick={() => setView("mailboxes")}>
            <span className="glyph">MB</span>
            <span>{t("rail.nav.mailboxes")}</span>
            <span className="count">{childMailboxes.length}</span>
          </button>
          <button className={view === "settings" ? "active" : ""} onClick={() => setView("settings")}>
            <span className="glyph">CFG</span>
            <span>{t("rail.nav.settings")}</span>
      ...
      <section className="work">
          ...
          <div className="actions">
            ...
            {view === "mailboxes" && (
              <button
                ...
                disabled={mailboxSyncBusy || !accounts.some((account) => account.status.hasDashboardCookie)}
        ...
        <div className="divider-ascii">路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路 路</div>
